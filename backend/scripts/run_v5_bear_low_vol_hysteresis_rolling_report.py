...
# -*- coding: utf-8 -*-
"""
Compara **legado** (sem overlay bear/vol), **flat** (overlay simples) e **histerese**
(bear = bench < MA252; entra com vol < p30 expansivo; sai com nao-bear OU vol > p55
durante N dias seguidos).

Para cada corrida V5 (moderado, cap 15%, mesmos precos), calcula janelas moveis 36m / 60m / 120m
...
actualizado (modo ``bear_low_vol_hysteresis``).

Uso (a partir de ``decide-core/backend``)::

    python scripts/run_v5_bear_low_vol_hysteresis_rolling_report.py
    python scripts/run_v5_bear_low_vol_hysteresis_rolling_report.py --json-out ..\\tmp_hysteresis_rolling.json
"""
...
def main() -> int:
    ...
    runs = {
        ...
        "flat_bear_low_vol": run_research_v1(
            **base_kw,
            ...
            bear_low_vol_hysteresis=False,
            bear_low_vol_exposure_mult=0.85,
        ),
        "hysteresis_bear_ma252_p30_p55_n10": run_research_v1(
            **base_kw,
            bear_low_vol_overlay_enabled=True,
            bear_low_vol_tiered=False,
            ...
            bear_low_vol_hysteresis_entry_quantile=0.30,
            bear_low_vol_hysteresis_exit_quantile=0.55,
            bear_low_vol_hysteresis_exit_consecutive_days=10,
            bear_low_vol_hysteresis_bear_ma_window=252,
        ),
        "hysteresis_bear_ma252_p35_p60_n10": run_research_v1(
            **base_kw,
            bear_low_vol_overlay_enabled=True,
            bear_low_vol_tiered=False,
            bear_low_vol_hysteresis=True,
        ...
        "hysteresis_bear_ma252_p40_p65_n10": run_research_v1(
            **base_kw,
            bear_low_vol_overlay_enabled=True,
            bear_low_vol_tiered=False,
            bear_low_vol_hysteresis=True,
    ...
    for name, r in runs.items():
        ...
        out_sum[name] = {
            ...
            "bear_low_vol_hysteresis": s.get("bear_low_vol_hysteresis"),
            "bear_low_vol_hysteresis_entry_edges": s.get("bear_low_vol_hysteresis_entry_edges"),
        }
    ...
    payload = {
        ...
        "rolling": out_roll,
        "notes": [
            "Histerese base: entrada vol < p30 expansivo, saida vol > p55 durante N=10 dias (ou nao-bear); mult 0.85.",
            "Opcao A (recomendada): entrada p40, saida p65, N=10; saida larga para reduzir pisca-pisca.",
        ],
    }
